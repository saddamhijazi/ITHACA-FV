...
double ITHACAutilities::error_fields_lInf(volVectorField& field1,
	volVectorField& field2)
{
	volScalarField magU = mag(field1-field2);
	volScalarField fieldMag1 = mag(field1);
	label buffmax = findMax(magU.internalField()); 
	double err = LInfnorm(magU) / fieldMag1[buffmax];
	return err;
}

Eigen::MatrixXd ITHACAutilities::error_listfields_lInf(PtrList<volVectorField>&
	fields1, PtrList<volVectorField>& fields2)
{
	...
	{
		err(k, 0) = error_fields_lInf(fields1[k], fields2[k]);
...
double ITHACAutilities::error_fields_lInf(volScalarField& field1,
	volScalarField& field2)
{
	volScalarField magP = mag(field1-field2);
	label buffmax = findMax(magP.internalField()); 
	double err = LInfnorm(magP) / mag(field1[buffmax]);
	return err;
}

Eigen::MatrixXd ITHACAutilities::error_listfields_lInf(PtrList<volScalarField>&
...
Eigen::MatrixXd ITHACAutilities::get_coeffs_ortho(PtrList<volScalarField>
	snapshots, PtrList<volScalarField>& modes)
{
	Eigen::MatrixXd coeff(modes.size(), snapshots.size());
...
Eigen::MatrixXd ITHACAutilities::get_coeffs_ortho(PtrList<volVectorField>
	snapshots, PtrList<volVectorField>& modes)
{
	Eigen::MatrixXd coeff(modes.size(), snapshots.size());
...
double ITHACAutilities::LInfnorm(volScalarField field)
{
	double a;
	a = gMax(field.internalField());
	return a;
}

double ITHACAutilities::LInfnorm(volVectorField field)
{
	double a;
	volScalarField magField = mag(field);
	a = LInfnorm(magField);
	return a;
}
