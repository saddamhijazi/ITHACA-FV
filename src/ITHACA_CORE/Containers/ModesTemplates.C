...
//             PtrList<GeometricField<T, fvPatchField, volMesh>> snapshots, PtrList<volScalarField> Volumes,
//             int numberOfModes,
//             word innerProduct)
// {
//     if (EigenModes.size() == 0)
...
//     PtrList<Eigen::VectorXd> totVolumes;
//     for (int i=0; i< Volumes.size(); i++)
//     {
//         Eigen::VectorXd volume = Foam2Eigen::field2Eigen(Volumes[i]);
//         totVolumes.append(&volume);
//     }
...
//         if (innerProduct == "L2")
//         {
//             M_vol = ITHACAutilities::get_mass_matrix_FV(totVolumes[i], "scalar");
//         }
...
//         M = Modes.transpose() * M_vol * Modes;
//         projSnapI = Modes.transpose() * M_vol * F_eigen;
...
//             word innerProduct)
// {
//     PtrList<GeometricField<T, fvPatchField, volMesh>> projSnap = projectSnapshots(snapshots, 0, innerProduct);
