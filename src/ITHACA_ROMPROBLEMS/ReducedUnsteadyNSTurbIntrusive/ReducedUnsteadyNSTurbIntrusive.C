        ...
        label startSnap)
{
    ...
    y.resize(Nphi_u, 1);
    y.setZero();
    //y = ITHACAutilities::get_coeffs(Usnapshots[startSnap], Umodes);
    y = initCond.col(0);
    int nextStore = 0;
    int counter2 = 0;
    ...
    fileName folder)
{
    ...
    Info << " online_solution.size() is " <<  online_solution.size() << endl;
    Info << " problem.nMatrix.rows are " <<  problem.nMatrix.rows() << endl;
