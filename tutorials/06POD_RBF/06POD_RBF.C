...
int main(int argc, char* argv[])
{
    ...
    word par_new("./par_online1"); // the samples which will be used in the online stage for checking the reduced order model.
    ...
    volScalarField average("Plift0",example.p);

    average = example.Pfield[0];

    for(unsigned int i = 1; i < example.Pfield.size(); i++ ){

    	average += example.Pfield[i]; 
    }

    average /= example.Pfield.size();
    Vector<double> Uinl(1, 0, 0);
    PtrList<volScalarField> PLiftfield;
    PLiftfield.append(average);
    ITHACAstream::exportFields(PLiftfield, "./ITHACAoutput/Offline", "Plift");
    ...
    //example.computeLiftT(example.Pfield, PLiftfield, example.Pomfield);

    ITHACAstream::exportFields(example.Uomfield, "./ITHACAoutput/Offline",
    	"Uofield");
    //ITHACAstream::exportFields(example.Pomfield, "./ITHACAoutput/Offline",
    //	"Pomfield");


    ITHACAPOD::getModes(example.Uomfield, example.Umodes, example.podex,
    	example.supex, 0, NmodesProject);
    //ITHACAPOD::getModes(example.Pomfield, example.Pmodes, example.podex,
    //	example.supex, 0, NmodesProject);
    ...
    Eigen::MatrixXd Mass_matrix_P = ITHACAutilities::get_mass_matrix(
    	example.Pmodes);
    Eigen::MatrixXd Mass_matrix_nut = ITHACAutilities::get_mass_matrix(
    	example.nuTmodes);
    ITHACAstream::exportMatrix(Mass_matrix_nut, "Mass_matrix_nut", "python",
    	"./ITHACAoutput/Matrices_L2/");
    ITHACAstream::exportMatrix(Mass_matrix_P, "Mass_matrix_P", "python",
    	"./ITHACAoutput/Matrices_L2/");
    example.projectSUP("./Matrices", NmodesU, NmodesP, NmodesSUP,
                       NmodesNUT); // Read the matrices if they already exist and if not compute them, you have to delete the folder Matrices if you want to re-do the computations with different number of modes
    reducedSteadyNSturb pod_rbf(
        example); //pod_rbf is an object of the class that used RBF in the online stage and thus takes into account the eddy viscosity in that stage
    pod_rbf.nu = 1e-3;
    ...
    reducedSteadyNS pod_normal(
        example); //pod_normal is an object of the classical POD model which does not use RBF in the online stage, here we want to make a comparison between the results of the two models
    pod_normal.nu = 1e-3;
